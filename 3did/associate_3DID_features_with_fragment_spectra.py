...
tdid_results_name = 'minvi-3000-2021-11-26-11-58-33'
features_dir = '/media/big-ssd/results-{}-3did/{}/features-3did'.format(tdid_experiment_name, tdid_results_name)
features_file = '{}/exp-{}-run-{}-features-3did-ident.feather'.format(features_dir, tdid_experiment_name, run_name)

INSTRUMENT_RESOLUTION = 40000.0

filter_by_mass_defect = False
...
def resolve_fragment_ions(feature_charge, ms2_points_df, mass_defect_bins):
...
@ray.remote
def associate_fragment_ions(cuboid, features_df):
    ...
    # get all the 3did features with an apex within these bounds
    ...
    if len(features_subset_df) > 0:
        ...
        # add the fragment ions to the feature
        features_subset_df['fragment_ions_l'] = features_subset_df.apply(lambda row: json.dumps(resolve_fragment_ions(row.charge, ms2_points_df, mass_defect_bins)), axis=1)
...
print('loaded {} detected features'.format(number_features_detected))
