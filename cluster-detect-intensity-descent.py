...
subprocess.call("del \\temp\\spectra*", shell=True)
...
for frame_id in range(args.frame_lower, args.frame_upper+1):
    ...
    while len(peaks_v) > 0:
        ...
        mgf_file_name = "\\temp\\spectra-frame-{}-peak-{}.mgf".format(frame_id, peak_id)
        hk_file_name = "\\temp\\spectra-frame-{}-peak-{}.hk".format(frame_id, peak_id)
        tmp_mgf_file_name = "\\temp\\spectra.mgf"
        tmp_hk_file_name = "\\temp\\spectra.hk"
        ...
        subprocess.call("copy /Y {} {}".format(mgf_file_name, tmp_mgf_file_name), shell=True)
        subprocess.call("C:\Hardklor-win64-2_3_0\Hardklor.exe .\Hardklor.conf", shell=True)
        subprocess.call("copy /Y {} {}".format(tmp_hk_file_name, hk_file_name), shell=True)
        ...
        cluster_quality = 0.0
        hk_df = pd.read_csv(hk_file_name, delimiter='\t', header=0, skiprows=1, skip_blank_lines=True, skipinitialspace=True,
