...
if args.new_mgf_spectra:
    ...
    if args.test_mode:
        ms1_deduped_df = ms1_deduped_df[:2]
    mgf_spectra_l = ray.get([deconvolute_ms2.remote(feature_df=feature_df, binned_ms2_for_feature=binned_ms2_df[binned_ms2_df.frame_id.isin([x[0] for x in feature_df.iloc[0].ms2_frames.values])], idx=idx, total=len(ms1_deduped_df)) for idx,feature_df in ms1_deduped_df.iterrows()])
