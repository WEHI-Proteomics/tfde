...
import sys
import peakutils
from ms_deisotope import deconvolute_peaks, averagine, scoring, deconvolution.peak_retention_strategy
...
def deconvolute_ms2_peaks_for_feature(feature_id, ms2_frame_id, binned_ms2_df):
    ...
    ms2_deconvoluted_peaks, _ = deconvolute_peaks(ms2_peaks_l, averagine=averagine.peptide, charge_range=(1,5), scorer=scoring.MSDeconVFitter(minimum_score=8, mass_error_tolerance=0.1), error_tolerance=4e-5, truncate_after=0.6, retention_strategy=peak_retention_strategy.TopNRetentionStrategy())
