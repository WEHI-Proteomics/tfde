...
mgf_file_name = "\\temp\\spectra-frame-{}-peak-{}.mgf".format(args.frame_id, args.peak_id)
hk_file_name = "\\temp\\spectra-frame-{}-peak-{}.hk".format(args.frame_id, args.peak_id)
...
if os.path.isfile(mgf_file_name) and os.path.isfile(hk_file_name) and os.path.isfile(args.database_name):
	...
	cluster_peaks_df = pd.read_sql_query("select peak_id,centroid_mz,intensity_sum from peaks where frame_id={} and cluster_id in (select cluster_id from peaks where frame_id={} and peak_id={}) order by peak_id asc;"
	    .format(args.frame_id, args.frame_id, args.peak_id), source_conn)

	f = plt.figure()
	ax1 = f.add_subplot(111)
	ax2 = ax1.twinx()
	plt.title("Database {}, frame {}, peak {}".format(args.database_name, args.frame_id, args.peak_id))
	ax1.set_xlabel('m/z')
