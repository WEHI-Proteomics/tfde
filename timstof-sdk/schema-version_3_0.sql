--
-- SQLite statements for setting up a "New TIMS data format" v3.1 database.
--
-- NOTE: THIS IS CURRENTLY THE OFFICIAL PRIMARY REFERENCE FOR THE DOCUMENTATION OF THIS
-- FILE FORMAT. If you mention this data format somewhere (Confluence, ...), please link
-- to this file in SVN in order to avoid multiple conflicting versions in the future.
--
-- Maintainer: Michael Krause, Sven Brehmer
-- 

-- List of global metadata describing this analysis.
-- 
-- There is a core set of "essential global metadata" that is always set in a valid TDF
-- analysis.
--
...
INSERT INTO GlobalMetadata VALUES ('SchemaVersionMinor', '1');
...
CREATE TABLE Frames (
    ...
    -- Maximum intensity occurring in all data belonging to this frame (can quickly
...
-- Bruker-internal information on m/z calibration for individual frames. For digitizer
-- delay / timebase information, refer to the global metadata.
...
-- Requirements:
--
-- (R1) precursors can be measured multiple times; the detailed precursor information
-- should be stored only once in the database, and not unncessarily repeated.
--
-- (R2) query all precursors found in a specified parent frame (for visualization; for
-- copying and converting data files)
--
-- (R3) query all frames in which specified precursor was measured (for merging the MS/MS
-- data together to improve SNR).
--
-- (R4) query all precursors measured in a specified PASEF MS^2 frame (for visualization
-- purposes)
--
-- (R5) post-processing software should be easily able to make MS/MS spectra not only from
-- ScanNumBegin to ScanNumEnd, but also by including all scans directly up to the adjacent
-- precursors.

-- One idea of this design: 'Precursors' table lists only the information derived from
-- MS^1 spectra. In contrast, how a precursor is actually measured (= quadrupole settings
-- and scan-number begin/end) remains in the 'PasefFrameMsMsInfo' table, even though this
-- will be repeated information for multiply measured precursors in the current version of
-- the scheduler (future versions might want to measure repeated precursors with slightly
-- different settings each?).

-- Table containing all precursors / features detected by an on-line precursor-selection
-- algorithm. This extra table avoids unnecessary repetitions (R1).
...
-- = 8, and no entry in the FrameMsMsInfo table.
--
